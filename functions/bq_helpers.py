...
def build_product_payload(product_id: str, data: dict) -> dict:
    ...
    payload = {
        ...
        PRODUCT_FIELDS.get("cost_price"): float(data.get("costPrice", 0)) if data.get("costPrice") is not None else None,
        ...
        PRODUCT_FIELDS.get("is_stock_tracked"): bool(data.get("isStockTracked", False)),
        PRODUCT_FIELDS.get("is_vat_applicable"): bool(data.get("isVatApplicable", False)),
        PRODUCT_FIELDS.get("last_updated"): data.get("lastUpdated").isoformat() if data.get("lastUpdated") else None,
        PRODUCT_FIELDS.get("original_price"): float(data.get("originalPrice", 0)) if data.get("originalPrice") is not None else None,
        ...
        PRODUCT_FIELDS.get("tag_labels"): data.get("tagLabels") if isinstance(data.get("tagLabels"), list) else None,
        PRODUCT_FIELDS.get("tags"): data.get("tags") if isinstance(data.get("tags"), list) else None,
        ...
        PRODUCT_FIELDS.get("vat_rate"): float(data.get("vatRate", 0)) if data.get("vatRate") is not None else None,
