...
# The HTTP `app_logs` function has been removed from this codebase.
#
# If you need to re-enable application logging via a Cloud Function,
# re-add an HTTP function here that accepts POST requests and performs
# best-effort writes to Cloud Logging and/or Firestore. Keep in mind
# this project previously provided a fault-tolerant `app_logs` endpoint
# which validated a minimal payload, optionally verified Firebase UIDs,
# and wrote structured logs to Google Cloud Logging and a Firestore
# collection named `appLogs`.
