...
def get_orders_by_date_bq(req: https_fn.Request) -> https_fn.Response:
    """Get orders within a date range from BigQuery"""
    ...
    to_date = req.args.get('to')
    store_id = req.args.get('storeId')
    uid = req.args.get('uid')  # Optional user filter
    ...
    if not from_date or not to_date:
        return https_fn.Response(
            json.dumps({"error": "from and to parameters are required (format: YYYYMMDD)"}),
            status=400,
            headers=DEFAULT_HEADERS
        )
    ...
    if not from_dt or not to_dt:
        return https_fn.Response(
            json.dumps({"error": "Invalid date format. Use YYYYMMDD"}),
            status=400,
            headers=DEFAULT_HEADERS
        )
    ...
    try:
        ...
        # Build orders query with date range filter
        query = f"""
        SELECT *
        FROM `{BIGQUERY_ORDERS_TABLE}`
        WHERE storeId = @store_id
        AND DATE(createdAt) BETWEEN @from_date AND @to_date
        """
        ...
        query += " ORDER BY createdAt DESC LIMIT 1000"  # Reasonable limit
        
        # Create job configuration with all parameters
        job_config = bigquery.QueryJobConfig(query_parameters=query_parameters)
        
        print(f"🔍 BigQuery orders by date query: {query}")
        print(f"📋 Parameters: store_id={store_id}, from_date={from_dt.date()}, to_date={to_dt.date()}, uid={uid}")
        ...
        print(f"❌ BigQuery orders by date query error: {str(e)}")
        return https_fn.Response(
            json.dumps({
                "success": False,
                "error": str(e),
